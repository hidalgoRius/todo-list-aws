...
class TestApi(unittest.TestCase):
    ...
    def test_api_translatetodo(self):
        ...
        #Test Translate TODO en idioma FR
        url = BASE_URL+"/todos/"+ID_TODO+"/fr"
        response = requests.get(url)
        json_response = response.json()
        print('Response Translate Todo French (fr): '+ str(json_response))
        self.assertEqual(
            response.status_code, 200, "Error en la petición API a {url}"
        )
        self.assertEqual(
        ...
        #Test Translate TODO en idioma EN
        url = BASE_URL+"/todos/"+ID_TODO+"/en"
        response = requests.get(url)
        json_response = response.json()
        print('Response Translate Todo French (en): '+ str(json_response))
        self.assertEqual(
            response.status_code, 200, "Error en la petición API a {url}"
        )
        self.assertEqual(
